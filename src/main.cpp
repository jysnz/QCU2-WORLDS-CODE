...
void initialize() {
  ...
  static pros::Task screen_task([]() {
    ...
    while (true) {
      ...
      // ── Header Display ──
      pros::screen::set_pen(ACCENT_CYAN);
      pros::screen::print(pros::E_TEXT_MEDIUM, 20, 12,
                          "CORE_OS // THERMAL_DYNAMICS");

      // ── Battery HUD ──
      double bat = pros::battery::get_capacity();
      uint32_t bat_col =
          (bat > 50) ? ACCENT_CYAN : (bat > 20 ? ACCENT_ORANGE : ACCENT_RED);
      ...
      auto drawTechCard = [&](int x, int y, const char *name, double temp) {
        ...
        // Geometric HUD bits
        pros::screen::set_pen(0x333344);
        pros::screen::draw_rect(x + 100, y + 28, x + w - 10,
                                y + 38); // Progress border

        double bar_pct = std::min(temp / 70.0, 1.0);
        pros::screen::set_pen(status_col);
        pros::screen::fill_rect(x + 102, y + 30,
                                x + 102 + (int)(bar_pct * (w - 114)), y + 36);
      ...
      // IMU Diagnostics
      bool imu_ok =
          !std::isnan(imu.get_heading()) && imu.get_heading() != PROS_ERR_F;
      bool imu_calib = imu.is_calibrating();
